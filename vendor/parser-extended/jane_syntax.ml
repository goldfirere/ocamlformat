...
module Comprehensions = struct
  ...
  *)
  ...
  let expr_of_clause_binding { pattern; iterator; attributes } =
    Ast_helper.Lb.mk ~attrs:attributes pattern (expr_of_iterator iterator) false
...
module Unboxed_constants = struct
  ...
  let of_constant { pconst_loc = loc; pconst_desc } = match pconst_desc with
  ...
  let of_expr expr =
    ...
    | Pexp_constant const -> of_constant const, expr.pexp_attributes
    | _ -> fail_malformed ~loc
  ...
  let of_pat pat =
    ...
    | Ppat_constant const -> of_constant const, pat.ppat_attributes
    | _ -> fail_malformed ~loc
  ...
  let expr_of ~loc ~attrs t =
    ...
    Expression.make_entire_jane_syntax ~loc feature (fun () ->
      Ast_helper.Exp.constant ~attrs (Ast_helper.Const.mk constant))

  let pat_of ~loc ~attrs t =
    let constant = constant_of t in
    Pattern.make_entire_jane_syntax ~loc feature (fun () ->
      Ast_helper.Pat.constant ~attrs (Ast_helper.Const.mk constant))
end
